...
const Header = ({ navLinks, enableAnalytics }) => {
  ...
  const handleLinkClick = (linkUrl, linkLabel) => {
    setActiveLink(linkUrl);

    if (enableAnalytics) {
      sendEvent('Nav Links', 'click', linkLabel);
    }
  };
