  ...
  return (
    <Layout
      meta={{
        ...
        pathname: `/tagged/${(tag || '').replace(/ /g, '+')}/page/${page}`,
      }}
    >
      <Posts
        posts={posts}
        heading={`Posts tagged ${tag} <br />– page ${page}`}
        pagination={pagination}
        paginationPath={`/tagged/${(tag || '').replace(/ /g, '+')}`}
        affiliate={affiliate}
      />
    </Layout>
  );
...
export async function getStaticProps({ params }) {
  ...
  if (
    Math.max(response.total_posts / 10) > parseInt(params.page) ||
