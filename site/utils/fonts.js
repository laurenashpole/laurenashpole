...
export async function findByIds(ids) {
  return await (await Font())
    .find({ active: true, _id: { $in: ids } })
    .sort({ name: 'asc' });
}
...
function getFontFileOptions(params, font, type) {
  return Object.keys(font[type]).reduce((obj, option) => {
    obj[option] = {
      ...font[type][option],
      is_included: params[type] ? !!params[type][option] : false,
    };
    return obj;
  }, {});
}
...
async function getFileParams(files, font) {
  ...
  const previewFiles = files.filter((file) => file.fieldname === 'font_files');

  const imageFileParams = await getSingleFileParams(
    imageFiles,
    font,
    'images/',
    'images',
  );
  const galleryFileParams = await getMultipleFileParams(
    galleryFiles,
    font,
    'images/',
    'images',
  ...
  const fontFileParams = await getSingleFileParams(
    fontFiles,
    font,
    'fonts/',
    'font_files',
  );
  const previewFileParams = await getMultipleFileParams(
    previewFiles,
    font,
    'previews/',
    'previews',
