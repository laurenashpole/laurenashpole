...
const Cart = () => {
  ...
  const handleAdd = (item) => {
    addItem(item);
    ga4Event('add_to_cart', [{ ...item, qty: 1 }], null, {
      value: cart.total + (item.sale_price || item.price),
    });
  };

  const handleRemove = (item, qty) => {
    removeItem(item, qty);
    ga4Event('remove_from_cart', [{ ...item, qty: qty }], null, {
      value: cart.total - (item.sale_price || item.price),
    });
  };
        ...
        {cart.total > 0 ? (
          ...
          <p>
            {checkoutState === 'complete'
              ? 'Order complete! Redirecting to your confirmation page...'
              : 'Your cart is empty.'}
          </p>
        )}
            ...
            {cart.total > 0 && (
              ...
              >
                <PayPalButtons
                  ...
                  forceReRender={[cart.count]}
                  onApprove={handleApprove}
                  onCancel={() => setCheckoutState('')}
                  onClick={handleClick}
                  onError={handleError}
                />
              </PayPalScriptProvider>
