        ...
        {font.images.gallery.map((image, i) => {
          return(
            <li key={i} className="gallery__thumb">
              <Button style="unstyled" onClick={() => setActiveIndex(i)} attributes={{ type: 'button', 'data-ga-click': true, 'data-ga-category': 'font page', 'data-ga-text': `View poster image ${i + 1}` }}>
                <Image src={`${process.env.NEXT_PUBLIC_ASSET_BASE_URL}${image}`} alt={`View poster image ${i + 1}`} width={360} height={240} />
              </Button>
            </li>
          );
        })}
